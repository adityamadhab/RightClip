import { BusFooter } from "../Business-Components/BusFooter";
import BusRatNav from "./BusRatNav";

const Card = ({ src, title }) => (
    <div className="rounded-md h-[250px] w-[280px] bg-white overflow-hidden flex flex-col">
                ...
                <div className="bg-[#FFEADD] p-14">
                    <div className="grid mt-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {cards.map((card, index) => (
                            <Card key={index} src={card.src} title={card.title} />
                        ))}
                    </div>
                </div>
            </div>
